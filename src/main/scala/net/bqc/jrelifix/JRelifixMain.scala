...
import net.bqc.jrelifix.faultlocalization.{JaguarConfig, JaguarLocalizationLibrary, PredefinedFaultLocalization}
...
//    val astParser = JavaParser()
//    astParser.batchParse(OptParser.params().sourceFolder, OptParser.params().classpath(), faultFiles)
//    logger.info("Done parsing AST!")
//    logger.info("Transforming faults to Java Nodes ...")
//    topNFaults.foreach(_.transformToJavaNode(astParser.globalASTs))
//    logger.info("Done Transforming!")
//    logger.info("Faults after transforming to Java Nodes: !")
//    topNFaults.take(OptParser.params().topNFaultLoc).foreach(logger.info(_))
//
...
//    val sourceFileContents: util.HashMap[String, DocumentASTRewrite] = SourceUtils.buildSourceDocumentMap(sourceFilesArray, sourcePath(0), astParser.globalASTs)
